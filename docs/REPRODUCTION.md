...
1. **A model** — the PEINN routing module (Emotion Engine + speech-act-aware Neutro Head v4
...
```bash
cd src
python -m peinn_v2.train.train --smoke   # OPTIONAL experimental energy module: pipeline wiring check
python -m peinn_v2.encoder.smoke_test    # OPTIONAL: encoder forward pass + gated combiner
```

These exercise the **optional** experimental structured-energy module (no downloads) to confirm a
torch training loop is wired correctly. They are not on the main routing path — the routing energy
is the frozen HybridCalibrator (§3).

## 3. Reproduce each component

The full recipe — data regeneration commands, judge prompts, hyper-parameters, output paths —
is in [`REGENERATE_CHECKPOINTS.md`](REGENERATE_CHECKPOINTS.md). In brief, the PEINN
components are:
...
The benchmark arms (H01–H17) and module plan live in `src/run_stat_batch.py`; the PEINN router
...
* a reachable base LLM. The self-contained backend in this repo is HuggingFace
  (`llm_backend: hf`); to use an Ollama or vLLM server, point its **OpenAI-compatible**
...
1. **Benchmarks are held out for evaluation only.** The six benchmarks (HarmBench, XSTest,
   ...
   keyword-overfitting OOD failure PEINN was built to avoid.
